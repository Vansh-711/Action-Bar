...
def handle_open_app(name):
    ...
    # MAC ONLY: Check if already focused
    if sys.platform == "darwin":
        try:
            ...
            if name.lower() in frontmost.lower():
                print(f"   ✨ '{name}' is already focused. Skipping launch.")
                return True
        except: pass
...
def handle_press_key(key):
    ...
    if "+" in key:
        ...
        time.sleep(0.2) # Increased delay for reliability
